...
本次套票版位於 `dist/ticket-types/MovieTicketAssistant.exe`，支援一般全票、特殊映演單人套票、指定座位及結帳會員登入。

繁體中文 Windows 桌面程式，使用 Python 3.11、Tkinter 與 Playwright。目標為 Windows 10／11 x64，電腦需已安裝 Microsoft Edge；執行打包版本不需安裝 Python。

## 使用

1. 開啟 `dist/ticket-types/MovieTicketAssistant.exe`（本次新版）。
2. 輸入起始網址，從下拉選單選影城，選擇場次的年／月／日及「第一場／最後一場」，設定票數、購票票種與是否自動勾選「我同意」。日期必須有效，日選單會隨月份與閏年調整。
...
建置：`powershell -ExecutionPolicy Bypass -File .\build.ps1 -OutputDirectory dist/ticket-types`。
...
- 「購票票種」可選「一般票種／全票」或「優惠套票／特殊映演單人套票」，並隨設定儲存。舊設定預設一般全票；執行期間鎖定選單。依提供範例，單人套票每份包含一張電影票，數量直接使用「購票張數」，座位數相同。
...
- 先以 `.panel-title` 完整文字比對設定票種的分類，再展開其 `.panel`；以內容的 class、實際高度及可見狀態判斷是否展開，不只依賴 aria-expanded。尚未展開時點擊標題並等待動畫，若事件無效則僅同步修正已核對內容的收合狀態。接著在該區塊 `table tr` 中，以 `td .spName` 或 `td` 完整文字比對票名，選擇該列可見的 `select.form-control-s`，以 option value 設定張數。例如套票模式只展開「優惠套票」，再選「特殊映演單人套票」。
- 分類、票名皆必須精確符合；不使用圖片標題或描述猜測，也不寫死 `collapse-1`、影廳代碼、票種 ID、data-hocode 或 data-pricecode。所選分類／票名不存在時停止，不改買其他票種；即使頁面只有套票區塊，也能啟動流程。多個同名區塊、同名票列或多個票數選單時停止提示手動確認；進階票數 CSS 僅在所選票列內生效。其他票種（包含收合區塊）已有張數時會停止，不混合購買。
...
採用資料驅動的 Registry（註冊表）搭配共用選票流程。`ticket_types.py` 的不可變 `TicketType` 定義分類與票名，`TICKET_TYPES` 以穩定識別碼註冊商品；設定檔儲存識別碼，UI 的顯示文字、驗證與網頁比對皆使用同一份定義。`automation.py` 的 `ticket_panel()`、`expand_ticket_panel()`、`select_quantity_and_continue()` 共用定位、展開、數量驗證、選取與繼續流程，不為每個商品複製一套邏輯。

新增相同操作方式、每份一張電影票的商品時，在註冊表新增一筆即可讓設定選單及選票流程使用它，再補上對應網頁範例測試。例如 `"another_single_package": TicketType("優惠套票", "另一種單人套票")`。兩種現有票券的操作行為相同，因此目前不為每種票券建立 Strategy 子類別；未來若有兌換碼、會員／信用卡資格欄位，或一份多席套票，再將實際不同的操作與座位數換算抽成策略，避免把差異散落於共用流程。

`#bookNormal` 是前一頁的購票入口規定，與數量頁的「一般票種」商品分類是兩個階段。此次範例新增的是數量頁商品，因此沿用既有入口規定，並在數量頁依設定選取商品。若未來票種需要不同入口表單，須再提供該頁範例以新增對應策略。

本次範例位於 `package_quantity_fixture.html`，保留提供的雙分類與套票名稱／描述，停用外部圖片載入並補上本機展開與繼續事件。`tests/test_ticket_types.py` 涵蓋兩種票券、動態 ID、缺少／重複商品、數量上限、其他票種已有數量及套票選位登入流程；測試不對真實網站購票。
